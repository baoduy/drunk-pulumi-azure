...
describe('Certificate tests', () => {
  it('Create Self-Sign Certificate', async () => {
    ...
    assert.notStrictEqual(cert.cert, undefined);
    assert.notStrictEqual(cert.privateKey, undefined);
  });

  it('Load P12 File Certificate', async () => {
    const cert = await certs.convertPfxFileToPem({
      certPath: path.resolve(`./src/z_tests/Certificate/local.p12`),
      password: '123456',
    });
  ...
  it('Load P12 Azure Key Vault File Certificate', async () => {
    const cert = await certs.convertPfxFileToPem({
      certPath: path.resolve(`./src/z_tests/Certificate/dev-root-vault-dev.pfx`),
    });
