...
import creator from '../../Builder/MySqlBuilder';
...
// Locker half is asserted instead: in MySqlBuilder.buildMySql both
...
const sku = { sku: { name: 'Standard_B1ms', tier: 'Burstable' }, version: '8.0.21' as const };

const buildMySql = () =>
  creator({ name: 'mysql', group, vaultInfo }).withSku(sku).withLogin({
    adminLogin: 'adminUser',
    password: 'P@ssw0rd123',
  });

describe('MySqlBuilder deletion guard (lock)', () => {
  it('non-production default: no .lock() call creates no ManagementLockByScope', async () => {
    const before = createdResources.length;
    buildMySql().build();

    // no lock resource is ever created on this path, so poll a couple of
    // ticks and confirm it stays absent rather than asserting synchronously.
  ...
  it('.lock() guards the instance with a CanNotDelete ManagementLockByScope', async () => {
    const before = createdResources.length;
    const rs = buildMySql().lock().build();
  ...
  it('lock() returns a chainable builder', () => {
    const rs = buildMySql().lock().withDatabases('appdb').build();
    assert.strictEqual(rs.name, naming.getMySqlName('mysql'));
  });
});

// There were no tests for MySqlBuilder before this cycle (DRK-1058), so the
// class-wide 80% coverage gate applies to the whole class, not just the new
// lock() surface — these cover the pre-existing chain methods the lock work
// touched the file around.
describe('MySqlBuilder other chain methods', () => {
  it('generateLogin() derives admin credentials instead of requiring withLogin', () => {
    const rs = creator({ name: 'mysql', group, vaultInfo })
      .withSku(sku)
      .generateLogin()
      .build();
  ...
  it('withOptions() overrides storage size and maintenance window on the server', async () => {
    const before = createdResources.length;
    creator({ name: 'mysql', group, vaultInfo })
      .withSku(sku)
      .withLogin({ adminLogin: 'adminUser', password: 'P@ssw0rd123' })
      .withOptions({
        storageSizeGB: 256,
        maintenanceWindow: { dayOfWeek: 1, startHour: 2, startMinute: 30 },
      })
      .build();

    const server = await waitForResource('azure-native:dbformysql:Server', before);
  ...
  it('withNetwork ipAddresses creates a whitelist FirewallRule', async () => {
    const before = createdResources.length;
    buildMySql().withNetwork({ ipAddresses: ['1.2.3.4'] }).build();

    const inputs = await waitForResource('azure-native:dbformysql:FirewallRule', before);
  ...
  it('withNetwork allowsPublicAccess creates the allow-public FirewallRule', async () => {
    const before = createdResources.length;
    buildMySql().withNetwork({ allowsPublicAccess: true }).build();
  ...
  it('withNetwork privateLink creates a private endpoint', async () => {
    const before = createdResources.length;
    buildMySql().withNetwork({ privateLink: { subnetIds: ['/subnet/1'] } }).build();
